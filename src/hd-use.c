   ...
   switch (weekday_of_date ((int)n, 9, year))
    {
      ...
      case DAY_MAX:
        ...
        break;
      case DAY_MIN:
        if (   (n-(int)n >= 23269.0/25920.0)
            && (g12 > 11))
          doy++;
        break;
      case 2:
        if (   (n-(int)n >= 1367.0/2160.0)
            && (g12 > 6))
          doy++;
        break;
      default:
...
*/
{
   ...
   */
   ...
   if (zq1 == (Ulint)SPECIAL_VALUE)
     return((int)zq1);
   zq1 = (Ulint)next_sun_longitude (&n, 60.0, (double)zq1, &d, &m, &y, hour, min);
   if (zq1 == (Ulint)SPECIAL_VALUE)
     return((int)zq1);
   /*
   ...
   */
   *nh_ws_prev_year=zq = (Ulint)next_sun_longitude (&n, 30.0, (double)zq_prev, &d, &m, &y, hour, min);
   ...
   */
   *conjunction_vector=cj = (Ulint)moonphase (MPHASE_NEW, FALSE, NULL, &the_conjunction,
                                              &d, &m, &y, hour, min);
   /*
   ...
   LOOP
    {
      ...
      */
      cj_prev = cj;
      cj = (Ulint)moonphase (MPHASE_NEW, FALSE, NULL, &the_conjunction, &d, &m, &y, hour, min);
      if (j)
       {
      ...
      */
      zq_prev = zq;
      zq = (Ulint)next_sun_longitude (&n, 30.0, (double)zq_prev, &d2, &m2, &y2, hour, min);
      if (zq == (Ulint)SPECIAL_VALUE)
        return((int)zq);
      /*
            ...
            switch (*format_txt)
             {
                       ...
                       if (j < k)
                        {
                          if ((Uint)len+k-j >= maxlen_max)
                            resize_all_strings (maxlen_max<<1, FALSE, __FILE__, (long)__LINE__);
                          for (i=0 ; i <= j ; i++)
                            *(*result_txt + len - i + k - j) = *(*result_txt + len - i);
                          for (i=0 ; i < k-j ; i++)
                            *(*result_txt + len - j + i) = ' ';
                          len += (k - j);
                        }
                    ...
                    if (!d)
                    ...
                    else
                     {
                       ...
                       if ((Uint)len+2+fwidth >= maxlen_max)
                         resize_all_strings (maxlen_max<<1, FALSE, __FILE__, (long)__LINE__);
                       strcat(*result_txt, "(");
                       len += use_format (result_txt, len+1, "", doy, TRUE,
                                          is_cformat, is_lformat, is_sign, is_lzero,
                                          FALSE, is_fformat, fstyle, fwidth, 0);
                       strcat(*result_txt, ")");
                       len += 2;
                     }
                  }
               ...
               case WDNAME_CHAR:
                 ...
                 len += use_format (result_txt, len, day_name (weekday_of_date (day, month, year)), 0, FALSE,
                                    is_cformat, is_lformat, is_sign, is_lzero,
                                    is_suffix, is_fformat, fstyle, fwidth, 0);
                 break;
               case MONTHNAME_CHAR:
                 if (!is_special_date)
                  {
                    ...
                    len += use_format (result_txt, len, month_name (month), 0, FALSE,
                                       is_cformat, is_lformat, is_sign, is_lzero,
                                       is_suffix, is_fformat, fstyle, fwidth, 0);
                  }
                 else
...
*/
{
   ...
   *is_cformat=(*is_lformat)=(*is_sign)=(*is_lzero)=(*is_suffix)=(*is_fformat) = FALSE;
   *fstyle = FSTYLE_NONE;
   *fwidth = SPECIAL_VALUE;
               ...
               if (is_suffix)
                {
                  ...
                  if (i <= 0)
                    ...
                    strncpy(*result_txt+pos, buf+len-fwidth-suffix_len, fwidth+suffix_len);
      ...
      do
       {
         ...
         */
         ...
         if (hdy_bahai)
           bahai_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL);
         if (hdy_celtic)
           celtic_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL);
         if (hdy_chinese_flexible)
           chinese_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL, TRUE);
         if (hdy_chinese)
           chinese_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL, FALSE);
         if (hdy_christian)
           christian_hdy (&init_data, detected, easter, year, &hd_elems, fday, count);
         if (hdy_hebrew)
           hebrew_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL);
         if (hdy_islamic)
           islamic_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL);
         if (hdy_japanese_flexible)
           japanese_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL, TRUE);
         if (hdy_japanese)
           japanese_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL, FALSE);
         if (hdy_multicultural_new_year)
           multicultural_new_year_hdy (&init_data, detected, year, &hd_elems, fday, count);
         if (hdy_orthodox_new)
           orthodox_hdy (&init_data, detected, easter, year, &hd_elems, fday, count, NULL, TRUE);
         if (hdy_orthodox_old)
           orthodox_hdy (&init_data, detected, easter, year, &hd_elems, fday, count, NULL, FALSE);
         if (hdy_persian)
           persian_hdy (&init_data, detected, year, &hd_elems, fday, count, NULL);
         ...
         if (mth_chinese_flexible)
           chinese_mth (&init_data, detected, year, &hd_elems, fday, count, TRUE, TRUE);
         if (mth_chinese)
           chinese_mth (&init_data, detected, year, &hd_elems, fday, count, FALSE, TRUE);
         if (mth_coptic)
           coptic_mth (&init_data, detected, year, &hd_elems, fday, count, TRUE);
         if (mth_ethiopic)
           coptic_mth (&init_data, detected, year, &hd_elems, fday, count, FALSE);
         ...
         if (mth_japanese_flexible)
           chinese_mth (&init_data, detected, year, &hd_elems, fday, count, TRUE, FALSE);
         if (mth_japanese)
           chinese_mth (&init_data, detected, year, &hd_elems, fday, count, FALSE, FALSE);
         if (mth_old_armenic)
           old_egyptic_mth (&init_data, detected, year, &hd_elems, fday, count, FALSE);
         if (mth_old_egyptic)
           old_egyptic_mth (&init_data, detected, year, &hd_elems, fday, count, TRUE);
         if (mth_persian)
              ...
              if (use_year_zeroleaded)
               {
                 if (is_leap_year)
                   sprintf(s1, _("%s:%*sThe year %0*d is A leap year"),
                           _("Eternal holiday list"), LEN_HD_NAME-i+2, "", lym, n);
                 else
              ...
              else
               {
                 if (is_leap_year)
                   sprintf(s1, _("%s:%*sThe year %d is A leap year"),
                           _("Eternal holiday list"), LEN_HD_NAME-i+2, "", n);
                 else
               ...
               && (hd < fday-(days_of_february (year-1)==29)))))
    {
      if (   holiday_flag
         ...
         )
       {
         ...
         if (   !i
             && (year == act_year))
           (void)decode_date_format (date_format->df_format, &s2, d, month, year, hd2,
                                     TRUE, FALSE, TRUE);
         else
