...
# if !HD_TOP20CC
  ...
  */
  ...
  { HD_ANNIVERSARY_OF_AMIRS_SUCCESSION,   N_("Anniversary of Amir's Succession") },
  ...
  { HD_ARCHBISHOP_MAKARIOS_MEMORIAL_DAY,  N_("Archbishop Makarios' Memorial Day") },
  ...
  { HD_GENERAL_SAN_MARTINS_ANNIVERSARY,   N_("General San Martin's Anniversary") },
...
*/
{
   ...
   */
   for (x=NH_VE ; x <= NH_WS ; x+=90.0)
    {
      mjd = equinox_solstice (x, &d, &m, &y, time_hour_offset, time_min_offset);
   ...
   if (year > 1843)
    {
      ...
      */
      sprintf(s2, "%s %d", _(hd_text[HD_BAHAI_NEW_YEARS_DAY].ht_text), year-1843);
      holiday (*init_data, detected, s2, ptr_cc_id,
               DIS_HLS_PREF, doy, 0, year, hd_elems, fday, count);
      if (*init_data)
        *init_data = FALSE;
      if (!cc_mode)
       {
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_FIRST_DAY_OF_RIDVAN].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+31, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_NINTH_DAY_OF_RIDVAN].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+39, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_TWELFTH_DAY_OF_RIDVAN].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+42, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_DECLARATION_OF_THE_BAB].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+63, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_ASCENSION_OF_BAHA_ULLAH].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+69, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_MARTYRDOM_OF_THE_BAB].ht_text),
                 ptr_cc_id, DIS_HLS_PREF, doy+110, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_BIRTH_OF_THE_BAB].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+213, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_BIRTH_OF_BAHA_ULLAH].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+236, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_DAY_OF_THE_COVENANT].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+250, 0, year, hd_elems, fday, count);
         holiday (*init_data, detected, _(hd_text[HD_BAHAI_ASCENSION_OF_ABDUL_BAHA].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, doy+252, 0, year, hd_elems, fday, count);
       }
    }
}
                 ...
                 if (i == 18)
                  {
                    ...
                    ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                   TRUE, bm, n-1843, SPECIAL_VALUE, hd_elems, fday, count);
                  }
                 else
                   ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                  FALSE, bm, n-1843, SPECIAL_VALUE, hd_elems, fday, count);
               }
              bdate += 19;
            }
         }
...
*/
{
   ...
   */
   if (sun_longitude (225.0, &d, &m, &y, time_hour_offset, time_min_offset, FALSE) != SPECIAL_VALUE)
    {
   ...
   if (!cc_mode)
    {
      if (sun_longitude (315.0, &d, &m, &y, time_hour_offset, time_min_offset, FALSE) != SPECIAL_VALUE)
       {
        ...
        y = year;
      if (sun_longitude (45.0, &d, &m, &y, time_hour_offset, time_min_offset, FALSE) != SPECIAL_VALUE)
       {
         holiday (*init_data, detected, _(hd_text[HD_CELTIC_BELTANE].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
         if (*init_data)
           *init_data = FALSE;
       }
        ...
        y = year;
      if (sun_longitude (135.0, &d, &m, &y, time_hour_offset, time_min_offset, FALSE) != SPECIAL_VALUE)
       {
         holiday (*init_data, detected, _(hd_text[HD_CELTIC_LUGHNASA].ht_text),
                  ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
         if (*init_data)
           *init_data = FALSE;
       }
         ...
         if (   !cc_mode
             ...
             || cc_mode == 10)
          {
            ...
            if (equinox_solstice (NH_WS, &d, &m, &y, hour, min) != SPECIAL_VALUE)
             {
               holiday (*init_data, detected, _(hd_text[HD_SOLSTICE_DAY].ht_text),
                        ptr_cc_id, hdy_prefix, d, m, y, hd_elems, fday, count);
               if (*init_data)
                 *init_data = FALSE;
             }
            else
         ...
         if (j)
          {
            if (   cc_mode == 3
              ...
              hdy_prefix = DIS_HLS_PREF;
            if (sun_longitude (15.0, &d, &m, &y, hour, min, FALSE) != SPECIAL_VALUE)
             {
               if (j == 1)
                 holiday (*init_data, detected, _(hd_text[HD_TOMB_SWEEPING_DAY].ht_text),
                          ptr_cc_id, hdy_prefix, d, m, y, hd_elems, fday, count);
               else
                {
                  (void)next_date (&d, &m, &y);
                  holiday (*init_data, detected, _(hd_text[HD_DAY_AFTER_TOMB_SWEEPING_DAY].ht_text),
                           ptr_cc_id, hdy_prefix, d, m, y, hd_elems, fday, count);
                }
               if (*init_data)
                 *init_data = FALSE;
...
LABEL_chinese_new_year_twice:
      ...
      */
      lm = find_chinese_leap_month (ptr_conjunction_vector, &zq, y, hour, min);
      if (lm == SPECIAL_VALUE)
       {
         /*
         ...
         */
         if (warning_level >= 0)
          {
#if USE_DE
            sprintf(s2, "Lunisolarer Kalender f%sr %d kann nicht korrekt ermittelt werden", UE, y);
#else /* !USE_DE */
            sprintf(s2, _("Cannot calculate lunisolar calendar for %d correctly"), y);
#endif /* !USE_DE */
            print_text (stderr, s2);
            if (warning_level >= WARN_LVL_MAX)
              my_error (ERR_INTERNAL_C_FUNC_FAILURE, __FILE__, ((long)__LINE__)-15L,
         ...
         */
         ...
         if (   !cc_mode
             ...
             || cc_mode == 16)
          {
            ...
            if (y == year)
             {
               holiday (*init_data, detected, _(hd_text[HD_CHINESE_NEW_YEARS_EVE].ht_text), ptr_cc_id,
                        (cc_mode) ? "+" : DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
               if (*init_data)
                 *init_data = FALSE;
             }
            (void)next_date (&d, &m, &y);
         ...
         if (y == year)
          {
            ...
            */
            k = y + 2637 + extra;
            sprintf(s2, "%s %d-%d", _(hd_text[HD_CHINESE_NEW_YEARS_DAY].ht_text),
                    k, (lm < 0) ? 0 : lm);
            holiday (*init_data, detected, s2, ptr_cc_id,
            ...
            if (   !cc_mode
                || cc_mode == SPECIAL_VALUE)
             {
               ...
               */
               i = ((k - 1) / 60) + 1;
               j = ((k - 1) % 60) + 1;
               sprintf(s2, "%s %d/%02d-%02d %s-%s", _(hd_text[HD_CHINESE_CYCLE].ht_text),
                       i, j, (lm < 0) ? 0 : lm, chi_stem[((j-1)%10)], _(chi_branch[((j-1)%12)]));
               holiday (*init_data, detected, s2, ptr_cc_id,
                        DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
             }
          }
         if (cc_mode != SPECIAL_VALUE)
          {
               ...
               for (i=0 ; i < j ; i++)
                {
                  ...
                  if (y == year)
                   {
                     holiday (*init_data, detected, _(hd_text[HD_CHINESE_NEW_YEARS_DAY].ht_text),
                              ptr_cc_id, hdy_prefix, d, m, y, hd_elems, fday, count);
                     if (*init_data)
                       *init_data = FALSE;
                   }
               ...
               for (i=0 ; i < j ; i++)
                {
                  if (y == year)
                   {
                     holiday (*init_data, detected, _(hd_text[HD_FESTIVAL_OF_LANTERNS].ht_text),
                              ptr_cc_id, hdy_prefix, d, m, y, hd_elems, fday, count);
                     if (*init_data)
                       *init_data = FALSE;
                   }
            ...
            if (!cc_mode)
             {
               ...
               if (y == year)
                {
                  holiday (*init_data, detected, _(hd_text[HD_LAO_TZES_BIRTHDAY].ht_text),
                           ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
                  if (*init_data)
                    *init_data = FALSE;
                }
            ...
            if (y == year)
             {
               ...
               if (   !cc_mode
                   ...
                   || cc_mode == 10)
                {
                  holiday (*init_data, detected, _(hd_text[HD_DRAGON_BOAT_FESTIVAL].ht_text),
                           ptr_cc_id, hdy_prefix, d, m, y, hd_elems, fday, count);
                  if (*init_data)
                    *init_data = FALSE;
                }
            ...
            */
            ...
            if (   (cc_mode == 6)
                && (y == year))
             {
               holiday (*init_data, detected, _(hd_text[HD_MID_AUTUMN_FESTIVAL].ht_text),
                        ptr_cc_id, hdy_prefix, d, m, y, hd_elems, fday, count);
               if (*init_data)
                 *init_data = FALSE;
             }
            ...
            if (   cc_mode == 3
                || cc_mode == 10)
             {
               ...
               if (y == year)
                {
                  holiday (*init_data, detected, _(hd_text[HD_DAY_AFTER_MID_AUTUMN_FESTIVAL].ht_text),
                           ptr_cc_id, "+", d, m, y, hd_elems, fday, count);
                  if (*init_data)
                    *init_data = FALSE;
                }
             }
            if (!cc_mode)
             {
               ...
               if (y == year)
                {
                  holiday (*init_data, detected, _(hd_text[HD_CONFUCIUS_BIRTHDAY].ht_text),
                           ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
                  if (*init_data)
                    *init_data = FALSE;
                }
            ...
            */
            ...
            if (   !cc_mode
                || cc_mode == 3)
             {
               ...
               if (y == year)
                {
                  holiday (*init_data, detected, _(hd_text[HD_DOUBLE_9_DAY].ht_text), ptr_cc_id,
                           (cc_mode) ? "+" : DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
                  if (*init_data)
                    *init_data = FALSE;
                }
            ...
            if (!cc_mode)
             {
               ...
               if (y == year)
                {
                  holiday (*init_data, detected, _(hd_text[HD_BODHIDHARMAS_BIRTHDAY].ht_text),
                           ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
                  if (*init_data)
                    *init_data = FALSE;
                }
                    ...
                    if (i == lm)
                     {
                       cm--;
                       ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                      TRUE, cm, j, SPECIAL_VALUE, hd_elems, fday, count);
                     }
                    else
                      ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                     FALSE, cm, j, SPECIAL_VALUE, hd_elems, fday, count);
                  }
             }
            if (cj_buf)
              if (cj_buf < conjunction_vector[cny])
               {
                 ...
                 if (y == year)
                   ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                  (lm_buf==MONTH_MAX), MONTH_MAX, j-1,
...
*/
{
   ...
   holiday (*init_data, detected, _(hd_text[HD_ASH_WEDNESDAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter-46, 0, year, hd_elems, fday, count);
   ...
   holiday (*init_data, detected, _(hd_text[HD_BOXING_DAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, 26, MONTH_MAX, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_CHRISTMAS_DAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, 25, MONTH_MAX, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_CHRISTMAS_EVE].ht_text),
            ptr_cc_id, DIS_HLS_PREF, 24, MONTH_MAX, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_CHRISTS_ASCENSION_DAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter+39, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_EASTER_MONDAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter+1, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_EASTER_SUNDAY].ht_text),
   ...
   holiday (*init_data, detected, _(hd_text[HD_FEAST_OF_CORPUS_CHRISTI].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter+60, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_FEAST_OF_HEART_JESUS].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter+68, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_GOOD_FRIDAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter-2, 0, year, hd_elems, fday, count);
   ...
   holiday (*init_data, detected, _(hd_text[HD_MARYS_ANNUNCIATION_DAY].ht_text),
   ...
   holiday (*init_data, detected, _(hd_text[HD_MARY_BLESSED_VIRGIN].ht_text),
            ptr_cc_id, DIS_HLS_PREF, DAY_MIN, MONTH_MIN, year, hd_elems, fday, count);
   ...
   holiday (*init_data, detected, _(hd_text[HD_MARYS_EXPECTATION].ht_text),
            ptr_cc_id, DIS_HLS_PREF, 18, MONTH_MAX, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_MARYS_IMMACULATE_CONCEPTION].ht_text),
            ptr_cc_id, DIS_HLS_PREF, 8, MONTH_MAX, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_MARYS_NATIVITY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, 8, 9, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_MARYS_NAME].ht_text),
            ptr_cc_id, DIS_HLS_PREF, 12, 9, year, hd_elems, fday, count);
   ...
   holiday (*init_data, detected, _(hd_text[HD_PALM_SUNDAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter-DAY_MAX, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_PASSION_SUNDAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter-14, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_PENTECOST].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter+49, 0, year, hd_elems, fday, count);
   ...
   if (year > 1516)
     holiday (*init_data, detected, _(hd_text[HD_REFORMATION_DAY].ht_text),
              ptr_cc_id, DIS_HLS_PREF, dvec[10-1], 10, year, hd_elems, fday, count);
   ...
   holiday (*init_data, detected, _(hd_text[HD_SEPTUAGESIMA_SUNDAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter-63, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_SEXAGESIMA_SUNDAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter-56, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_ST_ANDREWS_DAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, dvec[11-1], 11, year, hd_elems, fday, count);
   ...
   holiday (*init_data, detected, _(hd_text[HD_SYLVESTER].ht_text),
            ptr_cc_id, DIS_HLS_PREF, dvec[MONTH_MAX-1], MONTH_MAX, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_THREE_KINGS_DAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, 6, MONTH_MIN, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_HOLY_TRINITY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter+56, 0, year, hd_elems, fday, count);
   holiday (*init_data, detected, _(hd_text[HD_WHIT_MONDAY].ht_text),
            ptr_cc_id, DIS_HLS_PREF, easter+50, 0, year, hd_elems, fday, count);
}
           ...
           for (i=0 ; i <= MONTH_MAX ; i++)
            {
              ...
              if (y == year)
                ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year, (i==MONTH_MAX),
                               (i==MONTH_MAX) ? i : i+1, (do_coptic) ? n-283 : n-7,
                               SPECIAL_VALUE, hd_elems, fday, count);
              cdate += 30;
            }
         }
              ...
              for (i=0 ; i <= MONTH_MAX ; i++)
               {
                 ...
                 if (y == year)
                   ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                  (i==MONTH_MAX), (i==MONTH_MAX) ? i : i+1,
                                  n-1791, SPECIAL_VALUE, hd_elems, fday, count);
                 fdate += 30;
               }
            }
         }
           ...
           for (i=MONTH_MIN ; i <= MONTH_MAX ; i++)
            {
              ...
              if (y == year)
                ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                               FALSE, i, n-78, SPECIAL_VALUE, hd_elems, fday, count);
              if (i == MONTH_MIN)
...
*/
{
   ...
   */
   ...
   holiday (*init_data, detected, s2, ptr_cc_id,
            (cc_mode > 0) ? "+" : DIS_HLS_PREF, d, 0, year, hd_elems, fday, count);
   if (*init_data)
     *init_data = FALSE;
   if (cc_mode != SPECIAL_VALUE)
    {
      holiday (*init_data, detected, _(hd_text[HD_HEB_ROSH_HASHANA].ht_text), ptr_cc_id,
               (cc_mode) ? "+" : DIS_HLS_PREF, d+1, 0, year, hd_elems, fday, count);
      ...
      holiday (*init_data, detected, _(hd_text[HD_HEB_YOM_HAZIKARON].ht_text), ptr_cc_id,
               (cc_mode) ? "#" : DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
      d = doy - 162;
      if (!cc_mode)
        for (i=0 ; i < 7 ; i++)
          holiday (*init_data, detected, _(hd_text[HD_HEB_PESACH].ht_text),
                   ptr_cc_id, DIS_HLS_PREF, d+i, 0, year, hd_elems, fday, count);
      else
       {
         holiday (*init_data, detected, _(hd_text[HD_HEB_PESACH].ht_text),
      ...
      if (year > 1942)
       {
         (void)doy2date (doy-150, (days_of_february (year)==29), &d, &m);
         if (weekday_of_date (d, m, year) == 5)
           (void)doy2date (doy-151, (days_of_february (year)==29), &d, &m);
         holiday (*init_data, detected, _(hd_text[HD_HEB_YOM_HASHOAH].ht_text), ptr_cc_id,
                  (cc_mode) ? "#" : DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
       }
      ...
      if (i == 5)
        ...
        if (i == 6)
          (void)doy2date (doy-144, (days_of_february (year)==29), &d, &m);
      if (year > 1947)
        holiday (*init_data, detected, _(hd_text[HD_HEB_YOM_HA_AZMAUT].ht_text), ptr_cc_id,
                 (cc_mode) ? "+" : DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
      holiday (*init_data, detected, _(hd_text[HD_HEB_LAG_BOMER].ht_text),
               ptr_cc_id, DIS_HLS_PREF, doy-129, 0, year, hd_elems, fday, count);
      if (year > 1966)
        holiday (*init_data, detected, _(hd_text[HD_HEB_YOM_YERUSHALAYIM].ht_text),
                 ptr_cc_id, DIS_HLS_PREF, doy-119, 0, year, hd_elems, fday, count);
      holiday (*init_data, detected, _(hd_text[HD_HEB_SHAVUOT].ht_text), ptr_cc_id,
               (cc_mode) ? "+" : DIS_HLS_PREF, doy-112, 0, year, hd_elems, fday, count);
      ...
      holiday (*init_data, detected, _(hd_text[HD_HEB_TISHA_BAV].ht_text), ptr_cc_id,
               (cc_mode) ? "#" : DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
      holiday (*init_data, detected, _(hd_text[HD_HEB_YOM_KIPPUR].ht_text), ptr_cc_id,
               (cc_mode) ? "+" : DIS_HLS_PREF, doy+10, 0, year, hd_elems, fday, count);
      d = doy + 15;
      if (!cc_mode)
        for (i=0 ; i < 7 ; i++)
      ...
      holiday (*init_data, detected, _(hd_text[HD_HEB_SIMCHAT_TORAH].ht_text),
               ptr_cc_id, DIS_HLS_PREF, doy+22, 0, year, hd_elems, fday, count);
      /*
      ...
      for (n=year-1 ; n <= year ; n++)
       {
         ...
         */
         (void)doy2date (tishri_1 (n+1), (days_of_february (n+1)==29), &d, &m);
         ...
         */
         for (i=0 ; i < 8 ; i++)
          {
            ...
            if (y == year)
              holiday (*init_data, detected, _(hd_text[HD_HEB_HANNUKAH].ht_text),
                       ptr_cc_id, DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
          }
         num2date (hdate+hmvec[hm][5-1]+15, &d, &m, &y);
         if (y == year)
           holiday (*init_data, detected, _(hd_text[HD_HEB_TU_BSHEVAT].ht_text),
                    ptr_cc_id, DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
         ...
         if (y == year)
           holiday (*init_data, detected, _(hd_text[HD_HEB_SHUSHAN_PURIM].ht_text),
                    ptr_cc_id, DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
       }
    }
}
      ...
      if (doy2)
       {
         ...
         */
         check_twice = TRUE;
         sprintf(s2, "%s %d", _(hd_text[HD_ISL_NEW_YEARS_DAY].ht_text), y+1);
         holiday (*init_data, detected, s2, ptr_cc_id,
                  hdy_prefix, doy2, 0, year, hd_elems, fday, count);
       }
      /*
...
LABEL_ih_muharram_1_twice:
             ...
             */
                ...
                for (i=0 ; i < j ; i++)
                 {
                   ...
                   if (y == year)
                     holiday (*init_data, detected, _(hd_text[HD_ISL_ASHURA].ht_text),
                              ptr_cc_id, hdy_prefix, d, m, year, hd_elems, fday, count);
                 }
              }
             if (   (cc_mode != 3)
                 && (cc_mode != 4)
                ...
                for (i=0 ; i < j ; i++)
                 {
                   ...
                   if (y == year)
                     holiday (*init_data, detected, _(hd_text[HD_ISL_EID_I_MILAD_UN_NABI].ht_text),
                              ptr_cc_id, hdy_prefix, d, m, year, hd_elems, fday, count);
                 }
              }
             if (   !cc_mode
                 ...
                 || cc_mode == 69)
              {
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_SHAB_E_MIRAJ].ht_text), ptr_cc_id,
                           (cc_mode) ? "+" : DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
              }
             if (   !cc_mode
                 || cc_mode == 6)
              {
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_SHAB_E_BARAT].ht_text), ptr_cc_id,
                           (cc_mode) ? "+" : DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
              }
             if (   cc_mode == 10
                 || cc_mode == 43
             ...
             if (y == year)
               holiday (*init_data, detected, _(hd_text[HD_ISL_RAMADAN].ht_text),
                        ptr_cc_id, hdy_prefix, d, m, year, hd_elems, fday, count);
             if (   !cc_mode
                 || cc_mode == 6)
              {
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_SHAB_E_QADR].ht_text), ptr_cc_id,
                           (cc_mode) ? "+" : DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
              }
             if (   !cc_mode
                 || cc_mode == 62)
             ...
             for (i=0 ; i < j ; i++)
              {
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_EID_AL_FITR].ht_text),
                           ptr_cc_id, hdy_prefix, d, m, year, hd_elems, fday, count);
              }
             if (   !cc_mode
                 || cc_mode == 2
                 ...
                 || cc_mode == 43)
              {
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_WAQF_AL_ARAFAT].ht_text), ptr_cc_id,
                           (cc_mode) ? "+" : DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
              }
             if (   !cc_mode
                 || cc_mode == 26)
             ...
             for (i=0 ; i < j ; i++)
              {
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_EID_AL_ADHA].ht_text),
                           ptr_cc_id, hdy_prefix, d, m, year, hd_elems, fday, count);
              }
             if (cc_mode == 43)
              {
                ...
                */
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_HURAVEE_DAY].ht_text),
                           ptr_cc_id, "+", d, m, year, hd_elems, fday, count);
             ...
             if (!cc_mode)
              {
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_IMAMAT_DAY].ht_text),
                           ptr_cc_id, DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
                num2date (idate+170, &d, &m, &y);
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_AGA_KHANS_BIRTHDAY].ht_text),
                           ptr_cc_id, DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
                num2date (idate+252, &d, &m, &y);
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_NUZUL_AL_QURAN].ht_text),
                           ptr_cc_id, DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
                ...
                if (y == year)
                  holiday (*init_data, detected, _(hd_text[HD_ISL_QUDS_DAY].ht_text),
                           ptr_cc_id, DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
              ...
              if (y == year)
               {
                 /*
                    Calculate the year number used in the Islamic calendar.
                 */
                 (void)doy2date (j, (days_of_february (year)==29), &d2, &m2);
                 y = (int)ROUND(((date2num (d2, m2, n) - 227015L) / 354.36707) + 1.0);
                 ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                FALSE, i, y, SPECIAL_VALUE, hd_elems, fday, count);
               }
              idate += ((i & 1) ? 30 : 29);
            }
           if (   check_twice
      ...
      if (cc_mode != SPECIAL_VALUE)
       {
         /*
            Manage the major solar term/Zhong-Qi related festivals first.
         */
         if (equinox_solstice (NH_VE, &d, &m, &y, hour, min) != SPECIAL_VALUE)
          {
            holiday (*init_data, detected, _(hd_text[HD_EQUINOX_DAY].ht_text), ptr_cc_id,
                     (cc_mode) ? "+" : DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
            if (*init_data)
              *init_data = FALSE;
          }
         ...
         if (equinox_solstice (NH_WS, &d, &m, &y, hour, min) != SPECIAL_VALUE)
          {
            holiday (*init_data, detected, _(hd_text[HD_SOLSTICE_DAY].ht_text), ptr_cc_id,
                     (cc_mode) ? "+" : DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
            if (*init_data)
              *init_data = FALSE;
          }
         ...
         */
         if (sun_longitude (315.0, &d, &m, &y, hour, min, FALSE) != SPECIAL_VALUE)
          {
            (void)prev_date (&d, &m, &y);
            if (y == year)
             {
               holiday (*init_data, detected, _(hd_text[HD_SEASON_CHANGE].ht_text),
                        ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
               if (*init_data)
                 *init_data = FALSE;
             }
      ...
      else
       {
         ...
         */
         ...
         if (   (y == year)
             && (cc_mode != SPECIAL_VALUE))
          {
            holiday (*init_data, detected, _(hd_text[HD_JAPANESE_NEW_YEARS_EVE].ht_text),
                     ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
            if (*init_data)
              *init_data = FALSE;
          }
         ...
         if (y == year)
          {
            ...
            */
            k = y + 660 + extra;
            sprintf(s2, "%s %d-%d", _(hd_text[HD_JAPANESE_NEW_YEARS_DAY].ht_text),
                    k, (lm < 0) ? 0 : lm);
            holiday (*init_data, detected, s2, ptr_cc_id,
                     (cc_mode > 0) ? "+" : DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
            if (*init_data)
              *init_data = FALSE;
            if (   !cc_mode
                || cc_mode == SPECIAL_VALUE)
         ...
         if (cc_mode != SPECIAL_VALUE)
          {
            ...
            for (i=0 ; i < j ; i++)
             {
               (void)next_date (&d, &m, &y);
               if (y == year)
                 holiday (*init_data, detected, _(hd_text[HD_JAPANESE_NEW_YEARS_DAY].ht_text), ptr_cc_id,
                          (cc_mode) ? "+" : DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
             }
            /*
               Detect the date of the first day of the Japanese calendar
            ...
            */
            ...
            if (y == year)
             {
               holiday (*init_data, detected, _(hd_text[HD_MUSIC_WATER_BANQUET].ht_text),
                        ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
               if (*init_data)
                 *init_data = FALSE;
             }
            ...
            for (i=0 ; i < 3 ; i++)
             {
               if (y == year)
                {
                  holiday (*init_data, detected, _(hd_text[HD_FEAST_OF_FORTUNE].ht_text),
                           ptr_cc_id, DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
                  if (*init_data)
                    *init_data = FALSE;
                  (void)next_date (&d, &m, &y);
            ...
            */
            ...
            if (y == year)
             {
               holiday (*init_data, detected, _(hd_text[HD_MID_AUTUMN_FESTIVAL].ht_text), ptr_cc_id,
                        (cc_mode) ? "+" : DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
               if (*init_data)
                 *init_data = FALSE;
             }
...
*/
{
   ...
   chinese_hdy (init_data, detected, year, hd_elems, fday, count, HD_MCNY, FALSE);
   ...
   japanese_hdy (init_data, detected, year, hd_elems, fday, count, HD_MCNY, FALSE);
   orthodox_hdy (init_data, detected, 0, year, hd_elems, fday, count, HD_MCNY, TRUE);
   orthodox_hdy (init_data, detected, 0, year, hd_elems, fday, count, HD_MCNY, FALSE);
   ...
   if (year > 1791)
    {
      ...
      */
      ...
      if (mjd != (Ulint)SPECIAL_VALUE)
       {
         sprintf(s2, "%s %d", _(hd_text[HD_FRENCH_REVOLUTIONARY].ht_text), year-1791);
         holiday (*init_data, detected, s2, ptr_cc_id,
                  DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
       }
    }
   if (year > 1887)
    {
      ...
      */
      ...
      sprintf(s2, "%s %d", _(hd_text[HD_THAI_NEW_YEARS_DAY].ht_text), m+543);
      holiday (*init_data, detected, s2, ptr_cc_id,
   ...
   if (year > 1956)
    {
      ...
      */
      ptr_cc_id = "IN";
      sprintf(s2, "%s %d", _(hd_text[HD_INDIAN_NEW_YEARS_DAY].ht_text), year-78);
   ...
   if (year > 551)
    {
      ...
      */
      ...
      sprintf(s2, "%s %d", _(hd_text[HD_OLD_ARMENIC_NEW_YEARS_DAY].ht_text), y);
      holiday (*init_data, detected, s2, ptr_cc_id,
               DIS_HLS_PREF, (int)x, 0, year, hd_elems, fday, count);
      if (   (days_of_february (year) == 29)
          && ((int)x == 1))
       {
         sprintf(s2, "%s %d", _(hd_text[HD_OLD_ARMENIC_NEW_YEARS_DAY].ht_text), y+1);
         holiday (*init_data, detected, s2, ptr_cc_id,
                  DIS_HLS_PREF, DAY_LAST+1, 0, year, hd_elems, fday, count);
       }
    }
   /*
   ...
   */
   ...
   y = (int)x + 1;
   x = ROUND((1.0 - (x - (Ulint)x)) * (double)DAY_LAST);
   sprintf(s2, "%s %d-%d", _(hd_text[HD_OLD_EGYPTIC_NEW_YEARS_DAY].ht_text), y, m);
   holiday (*init_data, detected, s2, ptr_cc_id,
            DIS_HLS_PREF, (int)x, 0, year, hd_elems, fday, count);
   if (   (days_of_february (year) == 29)
       && ((int)x == 1))
    {
      sprintf(s2, "%s %d-%d", _(hd_text[HD_OLD_EGYPTIC_NEW_YEARS_DAY].ht_text), y+1, m);
      holiday (*init_data, detected, s2, ptr_cc_id,
               DIS_HLS_PREF, DAY_LAST+1, 0, year, hd_elems, fday, count);
    }
   /*
   ...
   */
   ...
   d = day_of_year (29, 8, year)
     + ((year & 3) == 3)
     + julian_gregorian_diff (29, 8, year);
   sprintf(s2, "%s %d", _(hd_text[HD_ETHIOPIC_NEW_YEARS_DAY].ht_text), year-7);
   holiday (*init_data, detected, s2, ptr_cc_id,
            DIS_HLS_PREF, d, 0, year, hd_elems, fday, count);
   if (year > 283)
    {
      ...
      */
      ptr_cc_id = "EG";
      sprintf(s2, "%s %d", _(hd_text[HD_COPTIC_NEW_YEARS_DAY].ht_text), year-283);
      holiday (*init_data, detected, s2, ptr_cc_id,
               DIS_HLS_PREF, d, 0, year, hd_elems, fday, count);
   }
}
        ...
        if (  do_egyptic
                ...
                && (n > 551)))
         {
           ...
           */
           if (do_egyptic)
            {
              mjd = date2num (DAY_MIN, MONTH_MIN, n) + MIN_BCE_TO_1_CE - 172235L;
              ...
              if (y == year)
               {
                 ...
                 ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                (em==MONTH_MAX), (em==MONTH_MAX) ? em : em+1, ey,
                                sothisperiod, hd_elems, fday, count);
               }
              edate += 30;
            }
           if (   (n == year)
               && (days_of_february (n) == 29)
               && ((int)mjd == 1))
             ocs_monthtext (init_data, detected, ptr_cc_id, DAY_LAST+1, 0, year,
                            FALSE, MONTH_MIN, ey+1, sothisperiod, hd_elems, fday, count);
         }
    }
}
   ...
   if (cc_mode == SPECIAL_VALUE)
   ...
   else
    {
      ...
      holiday (*init_data, detected, _(hd_text[HD_ASH_MONDAY].ht_text),
               ptr_cc_id, DIS_HLS_PREF, easter-48, 0, year, hd_elems, fday, count);
      if (*init_data)
        *init_data = FALSE;
      holiday (*init_data, detected, _(hd_text[HD_PALM_SUNDAY].ht_text),
               ptr_cc_id, DIS_HLS_PREF, easter-DAY_MAX, 0, year, hd_elems, fday, count);
      holiday (*init_data, detected, _(hd_text[HD_MAUNDY_THURSDAY].ht_text),
               ptr_cc_id, DIS_HLS_PREF, easter-3, 0, year, hd_elems, fday, count);
      holiday (*init_data, detected, _(hd_text[HD_GOOD_FRIDAY].ht_text),
               ptr_cc_id, DIS_HLS_PREF, easter-2, 0, year, hd_elems, fday, count);
      holiday (*init_data, detected, _(hd_text[HD_GOOD_SATURDAY].ht_text),
               ptr_cc_id, DIS_HLS_PREF, easter-1, 0, year, hd_elems, fday, count);
      ...
      if (   !is_new_calendar
          && (year > 1924))
        d += greg_diff;
      holiday (*init_data, detected, _(hd_text[HD_ORTHODOX_NEW_YEARS_DAY].ht_text),
      ...
      if (mjd != SPECIAL_VALUE)
       {
         ...
         */
         sprintf(s2, "%s %d", _(hd_text[HD_PERSIAN_NEW_YEARS_DAY].ht_text), year-621);
         holiday (*init_data, detected, s2, ptr_cc_id,
                  hdy_prefix, (int)mjd, 0, year, hd_elems, fday, count);
         if (*init_data)
           *init_data = FALSE;
         if (!cc_mode)
          {
            for (i=0 ; i < 5 ; i++)
             {
               holiday (*init_data, detected, _(hd_text[HD_MID_SPRING_FESTIVAL].ht_text),
                        ptr_cc_id, DIS_HLS_PREF, (int)mjd+40+i, 0, year, hd_elems, fday, count);
               holiday (*init_data, detected, _(hd_text[HD_MIDSUMMER_DAY].ht_text),
                        ptr_cc_id, DIS_HLS_PREF, (int)mjd+100+i, 0, year, hd_elems, fday, count);
               holiday (*init_data, detected, _(hd_text[HD_BRINGING_IN_THE_HARVEST_DAY].ht_text),
                        ptr_cc_id, DIS_HLS_PREF, (int)mjd+175+i, 0, year, hd_elems, fday, count);
               holiday (*init_data, detected, _(hd_text[HD_BRINGING_HOME_THE_HERDS_DAY].ht_text),
                        ptr_cc_id, DIS_HLS_PREF, (int)mjd+205+i, 0, year, hd_elems, fday, count);
             }
            /*
                 ...
                 if (mjd != SPECIAL_VALUE)
                  {
                    ...
                    */
                    ...
                    if (y == year)
                      holiday (*init_data, detected, _(hd_text[HD_FESTIVAL_OF_SADEH].ht_text),
                               ptr_cc_id, DIS_HLS_PREF, d, m, year, hd_elems, fday, count);
                  }
               }
          }
              ...
              for (i=MONTH_MIN ; i <= MONTH_MAX ; i++)
               {
                 ...
                 if (y == year)
                   ocs_monthtext (init_data, detected, ptr_cc_id, d, m, year,
                                  FALSE, i, n-621, SPECIAL_VALUE, hd_elems, fday, count);
                 pdate += ((i < 7) ? 31 : 30);
               }
            }
         }
...
*/
{
   ...
   */
   for (i=0,n=0.0 ; i < MONTH_MAX ; i++,n+=30.0)
    {
      mjd = sun_longitude (n, &d, &m, &y, time_hour_offset, time_min_offset, FALSE);
      if (mjd != SPECIAL_VALUE)
       {
         val2hours (mjd, &the_hour, &the_min, NULL);
         sprintf(s2, "ZhongQi-%02d/%s[%03d] %02d%s%02d", ((i+1)%MONTH_MAX)+1,
                 zod_name[i], (int)n, the_hour, time_sep, the_min);
         holiday (*init_data, detected, s2, ptr_cc_id,
                  DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
         if (*init_data)
           *init_data = FALSE;
      ...
      if (mjd != SPECIAL_VALUE)
       {
         ...
         sprintf(s2, "JieQi-%02d[%03d] %02d%s%02d",
                 ((i+2)%MONTH_MAX)+1, (int)n, the_hour, time_sep, the_min);
         holiday (*init_data, detected, s2, ptr_cc_id,
                  DIS_HLS_PREF, d, m, y, hd_elems, fday, count);
       }
      else
