...
*/
{
   ...
   */
   pressure /= 100.0;
   if (   pressure <= 0.0
       || degrees_altitude < -2.0
       || degrees_altitude >= DEGS_PER_06_HOURS)
     /*
   ...
   if (degrees_altitude < DEGS_PER_HOUR)
    {
      ...
      */
      for (i=0 ; i < 4 ; i++)
       {
         n = 1.0 / tan(TORAD(new_altitude+(7.31/(new_altitude+4.4))));
         new_degrees = n * new_pressure / (60.0 + new_temperature * (n + 39.0));
         ...
         */
         ...
         if (   (n != 0.0)
           ...
           */
           n = new_altitude - n * (degrees_altitude + new_degrees - new_altitude) / old_altitude;
         else
           /*
   ...
   */
   return(0.00007888888*pressure/((273.0+temperature)*tan(altitude)));
}
...
*/
{
   ...
   x1 = (1.0 / sqrt(cos2_gd_latitude+FLATTENING_OF_EARTH*sin2_gd_latitude))
...
*/
{
   ...
   */
   ...
   switch (coordinates->the_mode)
    {
      ...
      case 30:
        ...
        */
        return(date2num (day, month, year)+(MIN_BCE_TO_1_CE-1.5)+x1+time_offset);
      case 32:
        longitude = 0.0;
        break;
      case 33:
      ...
      case 36:
        ...
        the_time = sun_rise_set (event, is_limited, day, month, year, coordinates);
        switch (save_the_mode)
         {
           case 33:
           case 35:
           ...
           default:
             ...
             x4 = the_time - internal_moon_rise_set (event, day, month, year, coordinates);
             /*
      ...
      case 49:
        ...
        time_offset = sun_rise_set (event, is_limited, day, month, year, coordinates);
      ...
      default:
        longitude = coordinates->lon_deg
          + MM2DEG(coordinates->lon_min)
          + SS2DEG(coordinates->lon_sec);
    }
   ...
   if (coordinates->meters_above_sea_level > 0)
     geodetic_height = TORAD(0.0347 * sqrt(coordinates->meters_above_sea_level));
   /*
...
LABEL_sun_iter_mt:
      ...
      obliquity_of_ecliptic = TORAD(FIXANGLE(23.43929111
        - (0.013004167 + (0.00000016389 - 0.0000005036 * jc) * jc) * jc));
      argument_of_perihelion = TORAD(FIXANGLE(282.93735
        + (0.71953 + (0.00046 * jc)) * jc));
      mean_anomaly = TORAD(FIXANGLE(357.52910
        + (35999.05030 - (0.0001559 + 0.00000048 * jc) * jc) * jc));
      eccentricity = 0.016708617
        - (0.000042037 + (0.0000001236 * jc)) * jc;
      ...
      true_anomaly = sqrt(1.0-eccentricity*eccentricity) * sin(anomaly_of_eccentric);
      /*
         Calculate the geocentric Sun/Earth distance.
      */
      distance = sqrt(divisor_term*divisor_term+true_anomaly*true_anomaly);
      switch (coordinates->the_mode)
       {
         case 24:
           /*
      ...
      if (divisor_term < 0.0)
        true_anomaly += MY_PI;
      ecliptic_longitude = TORAD(FIXANGLE(TODEG(true_anomaly + argument_of_perihelion)
        + jc - 0.00569 - 0.00479 * sin(TORAD(124.90 - (1934.134 - (0.002063 * jc)) * jc))));
      ...
      */
      ...
      meridian_transit_time = FIXANGLE(right_ascension - local_sidereal_time);
      ...
      if (   (coordinates->the_mode >= 10)
          && (coordinates->the_mode <= 28))
       {
              ...
              if (coordinates->the_mode == 19)
                ...
                */
                return(TODEG(asin(sin_latitude * sin(declination)
                  + cos_latitude * cos(declination) * cos(x1))));
              /*
                 Return the Sun's geocentric apparent azimuth angle
                   in degrees plus fraction.
              */
              return(FIXANGLE(TODEG(my_atan2 (sin(x1),
                sin_latitude * cos(x1) - cos_latitude * tan(declination)) + MY_PI)));
            case 28:
              /*
            ...
            default:
              ...
              */
              x4 = gd_latitude2gc_latitude (latitude,
                                            (coordinates->meters_above_sea_level > 0)
                                            ? coordinates->meters_above_sea_level : 0,
         ...
         if (coordinates->the_mode == 13)
           /*
              Return the Sun's topocentric apparent ecliptic longitude
                in degrees plus fraction.
           */
           return(FIXANGLE(TODEG(my_atan2 (cos_obliquity_of_ecliptic * sin(TORAD(right_ascension))
             + sin_obliquity_of_ecliptic * tan(declination), cos(TORAD(right_ascension))))));
         /*
         ...
         */
         x1 = TORAD(FIXANGLE(local_sidereal_time - right_ascension));
         x3 = cos(x1);
         x2 = asin(sin_latitude*sin(declination)+cos_latitude*cos(declination)*x3);
         /*
         ...
         */
         x3 = FIXANGLE(TODEG(my_atan2 (sin(x1),
           sin_latitude*x3-cos_latitude*tan(declination)) + MY_PI));
         ...
         */
         ...
         /*
            Return the Sun's topocentric apparent right ascension angle
              in hours plus fraction.
         */
         return(DEG2HH(FIXANGLE(local_sidereal_time-TODEG(my_atan2 (x2, x3)))));
       }
      /*
        ...
        switch (save_the_mode)
         {
           ...
           case 38:
             if (save_the_mode == 36)
               ...
               */
               coordinates->the_mode = 19;
             the_time -= sun_rise_set (event, FALSE, day, month, year, coordinates);
             break;
           default:
             if (save_the_mode == 37)
               /*
               ...
               */
               coordinates->the_mode = 20;
             x4 = the_time - sun_rise_set (event, FALSE, day, month, year, coordinates);
             /*
                Reduce the resulting angle to -180.0...+180.0 degrees.
             */
             if (abs(x4) <= DEGS_PER_12_HOURS)
...
LABEL_moon_iter_mt:
         ...
         */
         ...
         argument_of_perihelion = TORAD(FIXANGLE(282.93735
           + (0.71953 + (0.00046 * jc)) * jc));
         sma_sun_mean_anomaly = TORAD(FIXANGLE(357.52910
           + (35999.05030 - (0.0001559 + 0.00000048 * jc) * jc) * jc));
         e_eccentricity = 0.016708617
           - (0.000042037 + (0.0000001236 * jc)) * jc;
         x2 = sma_sun_mean_anomaly + e_eccentricity
           * sin(sma_sun_mean_anomaly) * (1.0 + e_eccentricity * cos(sma_sun_mean_anomaly));
         do
          {
            x1 = x2;
            x2 = x1 - (x1 - e_eccentricity * sin(x1) - sma_sun_mean_anomaly)
              / (1.0 - e_eccentricity * cos(x1));
          } while (abs(abs(x1)-abs(x2)) > 0.000001);
         anomaly_of_eccentric = x2;
         divisor_term = cos(anomaly_of_eccentric) - e_eccentricity;
         s_distance = sqrt(1.0-e_eccentricity*e_eccentricity) * sin(anomaly_of_eccentric);
         true_anomaly = atan(s_distance/divisor_term);
         if (divisor_term < 0.0)
           true_anomaly += MY_PI;
         s_longitude = TORAD(FIXANGLE(TODEG(true_anomaly + argument_of_perihelion)
           + jc - 0.00569 - 0.00479 * sin(TORAD(124.90 - (1934.134 - (0.002063 * jc)) * jc))));
         /*
            Compute Moon data.
         */
         ...
         */
         ...
         x2 = 0.0039640 * sin(TORAD(FIXANGLE(346.560
           + (132.870 - (0.0091731 * jc)) * jc)));
         m_longitude = TORAD(FIXANGLE(259.183275
         ...
         */
         m_mean_longitude = FIXANGLE(270.434164
           + (481267.8831 - (0.001133 - 0.0000019 * jc) * jc) * jc
           + 0.000233 * x1
           + x2
           + 0.001964 * x3);
#if 0
         ...
         */
         ...
         mma_moon_mean_anomaly = TORAD(FIXANGLE(296.104608
           ...
           + x2
           + 0.002541 * x3));
         mme_moon_mean_elongation = TORAD(FIXANGLE(350.737486
           + (445267.1142 - (0.001436 - 0.0000019 * jc) * jc) * jc
           + 0.002011 * x1
           + x2
           + 0.001964 * x3));
         m_latitude = TORAD(FIXANGLE(11.250889
           + (483202.0251 - (0.003211 + 0.0000003 * jc) * jc) * jc
           + x2
           - 0.024691 * x3
           - 0.004328 * sin(m_longitude+x4)));
         ...
         */
         m_parallax = 0.950724
           ...
           + 0.000320 * cos(mma_moon_mean_anomaly-sma_sun_mean_anomaly) * e_eccentricity
           - 0.000271 * cos(mme_moon_mean_elongation)
           - 0.000264 * cos(sma_sun_mean_anomaly+mma_moon_mean_anomaly) * e_eccentricity
           - 0.000198 * cos(m_2_latitude-mma_moon_mean_anomaly)
           + 0.000173 * cos(mma_3)
           + 0.000167 * cos(ye)
           - 0.000111 * cos(sma_sun_mean_anomaly) * e_eccentricity
           + 0.000103 * cos(mme_4-mma_2)
           - 0.000084 * cos(mma_2-mme_2)
           - 0.000083 * cos(mme_2+sma_sun_mean_anomaly) * e_eccentricity
           + 0.000079 * cos(mme_2+mma_2)
           + 0.000072 * cos(mme_4)
           + 0.000064 * cos(x1+mma_moon_mean_anomaly) * e_eccentricity
           - 0.000063 * cos(mme_2+sma_sun_mean_anomaly-mma_moon_mean_anomaly) * e_eccentricity
           + 0.000041 * cos(sma_sun_mean_anomaly+mme_moon_mean_elongation) * e_eccentricity
           + 0.000035 * cos(mma_2-sma_sun_mean_anomaly) * e_eccentricity
           - 0.000033 * cos(mma_3-mme_2)
           - 0.000030 * cos(mma_moon_mean_anomaly+mme_moon_mean_elongation)
           - 0.000029 * cos(m_2_latitude-mme_2)
           - 0.000029 * cos(mma_2+sma_sun_mean_anomaly) * e_eccentricity
           + 0.000026 * cos(mme_2-sma_2) * e_e
         ...
         */
         m_latitude = 5.128189 * sin(m_latitude)
           ...
           + 0.003372 * sin(m_latitude-sma_sun_mean_anomaly-mme_2) * e_eccentricity
           + 0.002472 * sin(ze-sma_sun_mean_anomaly-mma_moon_mean_anomaly) * e_eccentricity
           + 0.002222 * sin(ze-sma_sun_mean_anomaly) * e_eccentricity
           + 0.002072 * sin(xg-sma_sun_mean_anomaly-mma_moon_mean_anomaly) * e_eccentricity
           + 0.001877 * sin(m_latitude-sma_sun_mean_anomaly+mma_moon_mean_anomaly) * e_eccentricity
           + 0.001828 * sin(yg-mma_moon_mean_anomaly)
           - 0.001803 * sin(m_latitude+sma_sun_mean_anomaly) * e_eccentricity
           - 0.001750 * sin(m_3_latitude)
           + 0.001570 * sin(mma_moon_mean_anomaly-sma_sun_mean_anomaly-m_latitude) * e_eccentricity
           - 0.001487 * sin(m_latitude+mme_moon_mean_elongation)
           - 0.001481 * sin(m_latitude+sma_sun_mean_anomaly+mma_moon_mean_anomaly) * e_eccentricity
           + 0.001417 * sin(m_latitude-sma_sun_mean_anomaly-mma_moon_mean_anomaly) * e_eccentricity
           + 0.001350 * sin(m_latitude-sma_sun_mean_anomaly) * e_eccentricity
           + 0.001330 * sin(m_latitude-mme_moon_mean_elongation)
           + 0.001106 * sin(m_latitude+mma_3)
           + 0.001020 * sin(yg)
           + 0.000833 * sin(zg-mma_moon_mean_anomaly)
           + 0.000781 * sin(mma_moon_mean_anomaly-m_3_latitude)
           + 0.000670 * sin(zg-mma_2)
           + 0.000606 * sin(mme_2-m_3_latitude)
           + 0.000597 * sin(mme_2+mma_2-m_latitude)
           + 0.000492 * sin(mme_2+mma_moon_mean_anomaly-sma_sun_mean_anomaly-m_latitude) * e_eccentricity
           + 0.000450 * sin(mma_2-m_latitude-mme_2)
           + 0.000439 * sin(mma_3-m_latitude)
           + 0.000423 * sin(m_latitude+mme_2+mma_2)
           + 0.000422 * sin(xg-mma_3)
           - 0.000367 * sin(sma_sun_mean_anomaly+m_latitude+mme_2-mma_moon_mean_anomaly) * e_eccentricity
           - 0.000353 * sin(sma_sun_mean_anomaly+m_latitude+mme_2) * e_eccentricity
           + 0.000331 * sin(zg)
           + 0.000317 * sin(ze-sma_sun_mean_anomaly+mma_moon_mean_anomaly) * e_eccentricity
           + 0.000306 * sin(mme_2-sma_2-m_latitude) * e_e
           - 0.000283 * sin(mma_moon_mean_anomaly+m_3_latitude)
           /* Nutation term. */
           * (1.0 - (0.0004664 * cos(m_longitude)) - (0.0000754 * cos(m_longitude+x4)));
         ...
         */
         m_longitude = m_mean_longitude
           ...
           + 0.041024 * sin(mma_moon_mean_anomaly-sma_sun_mean_anomaly) * e_eccentricity
           - 0.034718 * sin(mme_moon_mean_elongation)
           - 0.030465 * sin(sma_sun_mean_anomaly+mma_moon_mean_anomaly) * e_eccentricity
           ...
           - 0.007910 * sin(sma_sun_mean_anomaly-mma_moon_mean_anomaly+mme_2) * e_eccentricity
           - 0.006783 * sin(mme_2+sma_sun_mean_anomaly) * e_eccentricity
           + 0.005162 * sin(mma_moon_mean_anomaly-mme_moon_mean_elongation)
           + 0.005000 * sin(sma_sun_mean_anomaly+mme_moon_mean_elongation) * e_eccentricity
           + 0.004049 * sin(mma_moon_mean_anomaly-sma_sun_mean_anomaly+mme_2) * e_eccentricity
           + 0.003996 * sin(mma_2+mme_2)
           + 0.003862 * sin(mme_4)
           + 0.003665 * sin(mme_2-mma_3)
           ...
           - 0.002349 * sin(mma_moon_mean_anomaly+mme_moon_mean_elongation)
           + 0.002249 * sin(mme_2-sma_2) * e_e
           - 0.002125 * sin(mma_2+sma_sun_mean_anomaly) * e_eccentricity
           - 0.002079 * sin(sma_2) * e_e
           + 0.002059 * sin(x2-sma_2) * e_e
           - 0.001773 * sin(mma_moon_mean_anomaly+mme_2-m_2_latitude)
           - 0.001595 * sin(m_2_latitude+mme_2)
           + 0.001220 * sin(xe-mma_moon_mean_anomaly) * e_eccentricity
           - 0.001110 * sin(mma_2+m_2_latitude)
           + 0.000892 * sin(mma_moon_mean_anomaly-3.0*mme_moon_mean_elongation)
           - 0.000811 * sin(sma_sun_mean_anomaly+mma_moon_mean_anomaly+mme_2) * e_eccentricity
           + 0.000761 * sin(xe-mma_2) * e_eccentricity
           + 0.000717 * sin(mma_moon_mean_anomaly-sma_2) * e_e
           + 0.000704 * sin(mma_moon_mean_anomaly-sma_2-mme_2) * e_e
           + 0.000693 * sin(sma_sun_mean_anomaly-mma_2+mme_2) * e_eccentricity
           + 0.000598 * sin(x1-m_2_latitude) * e_eccentricity
           + 0.000550 * sin(mma_moon_mean_anomaly+mme_4)
           + 0.000538 * sin(4.0*mma_moon_mean_anomaly)
           + 0.000521 * sin(xe) * e_eccentricity
           + 0.000486 * sin(mma_2-mme_moon_mean_elongation)
           /* Nutation term. */
           + SS2DEG(-17.2 * x3 - 1.3 * sin(2.0*TORAD(FIXANGLE(279.6967+(36000.7689+(0.000303*jc))*jc))))
           /* Correction term for far away dates past/prior the epoch. */
           - 0.0005915 * jc * jc;
         ...
         */
         ...
         cos_obliquity_of_ecliptic = cos(obliquity_of_ecliptic);
         xe = xg;
         ye = cos_obliquity_of_ecliptic * yg - sin_obliquity_of_ecliptic * zg;
         ze = sin_obliquity_of_ecliptic * yg + cos_obliquity_of_ecliptic * zg;
         /*
            ...
            switch (coordinates->the_mode)
             {
               ...
               case 22:
                 ...
                 */
                 x2 = TODEG(x2);
                 s_distance = sqrt(divisor_term*divisor_term+s_distance*s_distance);
                 return(-21.62
                   + 5.0 * log10(s_distance*m_distance)
                   + 0.026 * x2
                   + 0.000000004 * x2 * x2 * x2 * x2);
               case 23:
               case 24:
                 x1 = TORAD(FIXANGLE(local_sidereal_time - right_ascension));
                 if (coordinates->the_mode == 23)
            ...
            switch (coordinates->the_mode)
             {
               ...
               case 17:
                 ...
                 */
                 m_longitude = FIXANGLE(TODEG(my_atan2 (cos_obliquity_of_ecliptic * sin(TORAD(right_ascension))
                   + sin_obliquity_of_ecliptic * tan(declination), cos(TORAD(right_ascension)))));
                 if (coordinates->the_mode == 13)
                   /*
                 ...
                 */
                 m_latitude = asin(cos_obliquity_of_ecliptic * sin(declination)
                   - sin_obliquity_of_ecliptic * cos(declination) * sin(TORAD(right_ascension)));
                 ...
                 */
                 ...
                 s_longitude = sun_rise_set (event, FALSE, day, month, year, coordinates);
                 coordinates->the_mode = save_the_mode;
                 x4 = acos(cos(TORAD(s_longitude-m_longitude))*cos(m_latitude));
                 ...
                 */
                 ...
                 s_distance = sun_rise_set (event, FALSE, day, month, year, coordinates);
                 coordinates->the_mode = save_the_mode;
                 return(-21.62
                   + 5.0 * log10(s_distance*m_distance)
                   + 0.026 * x2
         ...
         if (iter_mt)
          {
            ...
            */
            if (   (i > 3)
                && (abs(abs(meridian_transit_time)-abs(x4)) > DEGS_PER_12_HOURS))
              return(SPECIAL_VALUE);
            i++;
...
*/
{
   ...
   *is_full_new = FALSE;
   if (mjd <= 13L)
    {
      lunation = 1L;
      new_moon = moonphase (MPHASE_NEW, FALSE, NULL, &lunation, &day, &month, &year, hour, min);
      /*
   ...
   else
    {
      ...
      new_moon = moonphase (MPHASE_NEW, FALSE, NULL, &lunation, &day, &month, &year, -hour, -min);
      lunation--;
      full_moon = moonphase (MPHASE_FUL, FALSE, NULL, &lunation, &day, &month, &year, -hour, -min);
   ...
   if (mjd > (Ulint)new_moon)
     ...
     len_lunation_half = (-len_lunation_half + (mjd - full_moon)) / len_lunation_half;
